...
% (C) Copyright 2016                
%     All rights reserved           Bewersdorf Lab, Yale University
%
% Author: Emil B. Kromann, 2016
...
if (1 == 1)
    ...
    for nrow = 1:Nxyspace                                                   % for all rows
        [~, ncol1] = find(support(nrow,:) == 1, 1, 'first');                % find first pixel on pupil edge in current row
        [~, ncol2] = find(support(nrow,:) == 1, 1, 'last');                 % find last pixel on pupil edge in current row
    ...
    for ncol = 1:Nxyspace                                                   % for all cols
        [nrow1, ~] = find(support(:,ncol) == 1, 1, 'first');                % find first pixel on pupil edge in current col
        [nrow2, ~] = find(support(:,ncol) == 1, 1, 'last');                 % find last pixel on pupil edge in current col
