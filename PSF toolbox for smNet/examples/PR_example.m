...
basepath = pwd;
[FileDir, FileName] = fileparts([basepath,'\test data\complexPSF_recorded.mat']);
...
oprobj.PRobj.PRstruct.Lambda = 0.7;                 % center wavelength of the emission band pass filter, unit is micron
oprobj.PRobj.PRstruct.RefractiveIndex = 1.51;       % refractive index of the immersion medium
oprobj.PRobj.Pixelsize = 0.13;                      % pixel size on the sample plane, unit is micron
oprobj.PRobj.PSFsize = 128;                         % image size used for phase retrieval
oprobj.PRobj.SubroiSize = 50;                       % size of the cropped region from the recorded PSF data
oprobj.PRobj.OTFratioSize = 70;                     % image size used for 2D Gaussian fitting in OTF rescale
...
oprobj.PRobj.IterationNum = 40;                     % total iteration number used in phase retrieval algorithm
...
oprobj.PRobj.Enableunwrap = 1;                      % enable or disable phase unwrapping for Zernike expansion. Enable: 1, Disable: 0
...
resdir = fullfile(basepath,'/output/PR_result/');
if ~exist(resdir,'dir')
    mkdir(resdir)
end
