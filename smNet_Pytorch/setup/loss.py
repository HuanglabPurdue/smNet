...
class CRLBMSELoss(_Loss):

    def __init__(self, size_average=True, reduce=True):
        super(CRLBMSELoss, self).__init__(size_average)
        self.reduce = reduce

    def forward(self, input, target, weight):
        _assert_no_grad(target)
        _assert_no_grad(weight)
        return F.mse_loss(torch.div(input, weight), torch.div(target, weight), size_average=self.size_average, reduce=self.reduce)
